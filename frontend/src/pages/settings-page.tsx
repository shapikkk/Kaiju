    ...
    preview: (
      <div className="h-10 w-full rounded-md border bg-white">
        ...
        <div className="flex gap-1 p-1.5">
          ...
          <span className="h-3 w-8 rounded bg-gray-100" />
        </div>
      </div>
    ),
  },
  {
    ...
    preview: (
      <div className="h-10 w-full rounded-md border border-zinc-700 bg-zinc-900">
        ...
        <div className="flex gap-1 p-1.5">
          <span className="h-3 w-6 rounded bg-zinc-700" />
          ...
          {THEME_OPTIONS.map((option) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'flex flex-col gap-3 rounded-xl border-2 p-4 text-left transition-all hover:bg-accent/50',
                  isActive
      ...
      <div className="space-y-3">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Sets the background tone for the entire interface.
          </p>
        </div>
          ...
          {BASE_OPTIONS.map((opt) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'relative flex flex-col items-start gap-2 rounded-lg border-2 p-3 text-left transition-all hover:border-primary/60',
                  isActive
              ...
              >
                ...
                >
                  <div
                    className="m-1.5 h-2 w-1/2 rounded-sm opacity-60"
                    style={{ background: opt.border }}
                  />
                  <div
                    className="mx-1.5 h-1.5 w-3/4 rounded-sm opacity-40"
                    style={{ background: opt.border }}
                  />
                </div>
          ...
          {ACCENT_OPTIONS.map((opt) => {
            ...
            return (
              ...
              >
                <span
                  className={cn(
                    'flex h-9 w-9 items-center justify-center rounded-full transition-all ring-offset-background ring-offset-2',
                    isActive
    ...
    <div className="space-y-8">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Your name, email, title, and bio are visible to your team.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-5">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <Input
                id="pf-name"
                value={effectiveName}
                onChange={(e) => { setName(e.target.value); setInfoMsg(null); }}
          ...
          <div className="space-y-1.5">
            <Label htmlFor="pf-job">Role / Job Title</Label>
            <Input
              id="pf-job"
              value={effectiveJobTitle}
              onChange={(e) => { setJobTitle(e.target.value); setInfoMsg(null); }}
          ...
          <div className="space-y-1.5">
            ...
            <Textarea
              id="pf-bio"
              value={effectiveBio}
              onChange={(e) => { setBio(e.target.value); setInfoMsg(null); }}
          ...
          <div className="flex justify-end border-t pt-4">
            <Button onClick={handleSaveInfo} disabled={!infoChanged || infoIsPending}>
              {infoIsPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              Save Changes
            </Button>
          </div>
        </CardContent>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-1.5">
            <Label htmlFor="pf-curr-pwd">Current Password</Label>
            <Input
              id="pf-curr-pwd"
              type="password"
              value={currentPwd}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              <Label htmlFor="pf-confirm-pwd">Confirm New Password</Label>
              <Input
                id="pf-confirm-pwd"
                type="password"
                value={confirmPwd}
          ...
          <div className="flex justify-end border-t pt-4">
            <Button
              ...
              disabled={
                !currentPwd || !newPwd || !confirmPwd || updatePassword.isPending
              }
            >
...
function NotificationsTab() {
  ...
  const save = (patch: Parameters<typeof updateNotifPrefs.mutate>[0]) => {
  ...
  return (
    <div className="space-y-8">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Activity summaries and important updates sent to your inbox.
              </p>
            </div>
            <Switch
              checked={emailNotifs}
              onCheckedChange={(v) => { setEmailNotifs(v); save({ email: v }); }}
            />
          </div>
          <Separator />
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Real-time alerts in your browser or desktop app.
              </p>
            </div>
            <Switch
              checked={pushNotifs}
              onCheckedChange={(v) => { setPushNotifs(v); save({ push: v }); }}
            />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Activity Triggers</CardTitle>
          <CardDescription>Choose which events generate a notification for you.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                When a task is assigned or re-assigned to you.
              </p>
            </div>
            <Switch
              checked={assignedNotif}
              onCheckedChange={(v) => { setAssignedNotif(v); save({ assigned: v }); }}
            />
          </div>
          <Separator />
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-sm font-medium">Comments on My Tasks</p>
              <p className="text-xs text-muted-foreground">
                When someone comments on a task you own or are watching.
              </p>
            </div>
            <Switch
              checked={commentsNotif}
              onCheckedChange={(v) => { setCommentsNotif(v); save({ comments: v }); }}
            />
          </div>
          <Separator />
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Get reminded 24 hours before a task you own is due.
              </p>
            </div>
            <Switch
              checked={dueNotif}
              onCheckedChange={(v) => { setDueNotif(v); save({ due_date: v }); }}
            />
          </div>
        </CardContent>
      </Card>
      ...
      <div className="space-y-4">
        ...
        <div className="flex h-40 items-center justify-center rounded-xl border border-dashed">
          <p className="text-sm text-muted-foreground">
            Navigate to a workspace first to manage its settings.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-xl font-semibold text-foreground">Workspace</h2>
        <p className="mt-1 text-sm text-muted-foreground">
          Manage settings for <strong>{workspace.name}</strong>.
        </p>
      </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-[11px] text-muted-foreground">
                  PNG, JPG up to 2 MB. Coming soon.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-end border-t pt-4">
            <Button
              onClick={() => updateWorkspace.mutate({ name: effectiveName.trim() })}
              disabled={
                !effectiveName.trim() ||
                effectiveName.trim() === workspace.name ||
          ...
          <div className="flex items-start justify-between gap-4 rounded-lg border border-destructive/30 bg-destructive/5 p-4">
            <div>
              ...
              <p className="mt-0.5 text-sm text-muted-foreground">
                Once deleted, all boards, tasks, and data will be permanently
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              <strong className="text-foreground">{workspace.name}</strong>? This
              action cannot be undone. All boards, tasks, sprints, epics, tags,
          ...
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() => deleteWorkspace.mutate(workspaceSlug)}
            >
